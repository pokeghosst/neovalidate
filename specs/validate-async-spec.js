describe('validate.async', function () {
  ...
  describe('waitForResults', function () {
    ...
    it.promise('handles results with no promises', function () {
      const results = [{ attribute: 'foo', error: 'bar' }]
    ...
    it.promise(
      'rejects the promise if any promise throw an exception',
      function () {
        const results = [
          {
            attribute: 'foo',
            error: new validate.Promise(function (res, rej) {
              res()
            })
          },
          {
  ...
  it.promise(
    'rejects the promise with an error if an exception is thrown',
    function () {
      const c = {
        attribute: {
          asyncError: true
  ...
  it.promise('cleans the attributes per default', function () {
    const attrs = { foo: 'bar' }
    const constraints = { bar: { presence: true } }
    const cleaned = { bar: 'foo' }
    ...
    return validate
      ...
      .then(function () {
        ...
        expect(validate.cleanAttributes).toHaveBeenCalledWith(
          attrs,
          constraints
        )
      })
  ...
  it.promise(
    "doesn't cleans the attributes is cleanAttributes: false",
    function () {
      const attrs = { foo: 'bar' }
      const constraints = { foo: { presence: true } }
      const cleaned = { bar: 'foo' }
      ...
      return validate
        .async(attrs, constraints, { cleanAttributes: false })
        .then(success, error)
        .then(function () {
          expect(error).not.toHaveBeenCalled()
          expect(success).toHaveBeenCalledWith(attrs)
  ...
  describe('wrapping errors', function () {
    it.promise(
      'it allows you to wrap errors using a custom function',
      function () {
        ...
        var wrapper = jasmine
          .createSpy('wrapper')
          .and.callFake(function (errors, options, attributes, constraints) {
        ...
        return validate
          .async(attrs, originalConstraints, originalOptions)
          .then(success, error)
          .then(function () {
    ...
    it.promise('calls the wrapper function with the new keyword', function () {
      var wrapper = jasmine
        .createSpy('wrapper')
        .and.callFake(function (errors) {
          expect(this.constructor).toBe(wrapper)
          return errors
        })

      return validate
        .async({}, { foo: { presence: true } }, { wrapErrors: wrapper })
        .then(success, error)
        .then(function () {
          expect(error).toHaveBeenCalled()
          expect(success).not.toHaveBeenCalled()

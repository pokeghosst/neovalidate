describe('validator.length', function () {
  const length = validate.validators.length.bind(validate.validators.length)
  ...
  it('supports default options', function () {
    const tokenizer = jasmine.createSpy('tokenizer').and.returnValue({ length: 3 })
  ...
  it('allows functions as messages', function () {
    ...
    const options = { message, minimum: 10 }
    const value = 'foo'
